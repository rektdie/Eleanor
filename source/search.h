...
class ContHistory {
private:
    // indexed by [other color][prev move piece][stm][prev move to][piece][to][threatenedSource][threatenedTarget]
    MultiArray<int16_t, 2, 6, 64, 2, 6, 64, 2, 2> contHistMoves;
public:
    void Update(bool stm, bool otherColor, int prevType, int prevTo, int type, int to, bool source, bool target, int bonus) {
        int clampedBonus = std::clamp(bonus, -MAX_HISTORY, MAX_HISTORY);

        contHistMoves[otherColor][prevType][prevTo][stm][type][to][source][target] +=
            clampedBonus - contHistMoves[otherColor][prevType][prevTo][stm][type][to][source][target] * std::abs(clampedBonus) / MAX_HISTORY;
    }

    void Clear() {
        std::fill(&contHistMoves[0][0][0][0][0][0][0][0], &contHistMoves[0][0][0][0][0][0][0][0] + sizeof(contHistMoves) / sizeof(int16_t), 0);
    }
