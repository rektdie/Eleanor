...
template <bool isPV>
static int GetReductions(Board &board, Move &move, int depth, int moveSeen, int ply, bool cutnode, bool improving, bool corrplexity, bool ttpv, bool ttpvFailLow, bool ttNoisy, SearchContext* ctx) {
    int reduction = 0;
...
SearchResults PVS(Board& board, int depth, int alpha, int beta, int ply, SearchContext* ctx, bool cutnode) {
    ...
    while ((currMove = mp.Next())) {
        ...
        if (!isPV && ply && currMove.IsQuiet()
                && lmrDepth <= 5 && staticEval + margin < alpha && notMated && !board.GivesDirectCheck(currMove)) {
            continue;
        }
        ...
        const bool ttNoisy = !entry.bestMove.IsQuiet();

        int reductions = GetReductions<isPV>(board, currMove, depth, moveSeen, ply, cutnode, improving, corrplexity, ttpv, ttpvFailLow, ttNoisy, ctx);
